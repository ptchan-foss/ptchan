<?php
	...
	class RecentPosts {
		public function build($action, $settings) {
			...
			if ($action == 'all') {
				copy('templates/themes/ptchan_frontpage/' . $settings['basecss'], $config['dir']['home'] . $settings['css']);
			}

			$this->excluded = explode(' ', $settings['exclude']);

			if ($action == 'all' || $action == 'post' || $action == 'post-thread' || $action == 'post-delete' || $action == 'news') {
		...
		public function homepage($settings) {
			...
			// News
			$query = query("SELECT * FROM ``news`` ORDER BY `time` DESC" . ($settings['news_entries'] ? ' LIMIT ' . $settings['news_entries'] : '')) or error(db_error());
			...
			foreach($config['boards'] as $key => $group) {
				foreach($group as $board) {
					if (isset ($xboards[$board])) {
						$board_groups[$key][] = $xboards[$board];
					}
				}
			}

			return Element('themes/ptchan_frontpage/index.html', Array(
				'settings' => $settings,
				'config' => $config,
